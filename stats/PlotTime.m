...
%set(gca, 'position', [xstart, ystart, xdim+0.2, ydim]);
...
markersColor = [1 0 1;0.5 0.5 0.5;0.6 0.6 1;0.4 0.8 0.4;0.1 0.4 0.6;0.9 0.2 0.1;0.3 0.1 0.5];
markers={'o','s','d','v','^','h','x'};
markerSize = 10;
fontsize   = 15;
linewidth  = 2.0;
...
for i = 1:size(sceneName,2)
  if strcmp(sceneName{1,i}(1,1),'s') == 1
    ...
    StatData(6) = {ReadResults(char(strcat(sceneName(1,i),sceneNamePath{robotType,COOP2})))};
    StatData(7) = {ReadResults(char(strcat(sceneName(1,i),sceneNamePath{robotType,COOP10})))};
    ...
    for p = 1:5
      plot (rstart:1:rend,StatData{p}(1:size(StatData{1},1), I_TOTAL_TIME),...
      'marker',markers{p},...
      'color',markersColor(p,:),...
      'markerfacecolor', markersColor(p,:),...
  ...
  if strcmp(sceneName{1,i}(1,1),'c') == 1
    ...
    for p = 4:5
      plot ([rstart],StatData{p}(1:size(StatData{p},1), I_TOTAL_TIME),...
      'marker',markers{p},...
      'color',markersColor(p,:),...
      'markerfacecolor', markersColor(p,:),...
      'markeredgecolor', markersColor(p,:),...
    ...
    for p = 4:5
      y = StatData{p}(1:size(StatData{p},1), I_TOTAL_TIME);
      x = rstart:1:rend;
      ystd = StatData{p}(1:size(StatData{p},1), I_TOTAL_TIME_STD);
      for k = 1 : 1 : length(ystd)
...
%legend(plannerlabels{1,COOP0},plannerlabels{1,COOP5},plannerlabels{1,COOP2},plannerlabels{1,COOP10});
