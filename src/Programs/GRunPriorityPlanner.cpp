...
namespace MP
{
  class GRunPriorityPlanner : public GManager
  {
  public:
    enum
    {
    ...
    GRunPriorityPlanner(void) : GManager()
    {
      m_timer   = 50;
    ...
    virtual ~GRunPriorityPlanner(void)
    {
    ...
    virtual bool HandleEventOnNormalKeyPress(const int key)
    {
      ...
      printf("check = %f\n", Stats::GetSingleton()->GetValue("check"));
...
extern "C" int GRunPriorityPlanner(int argc, char **argv)
{
  MP::GRunPriorityPlanner gManager;
